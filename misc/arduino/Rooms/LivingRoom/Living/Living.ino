...
void loop(void) {
  ...
  if (motion_value == 1){
    ...
    if (MOTIONVALUE == 0){
      sprintf(buf, "%sdevice[]=%s&value[]=%i&", buf, "motion1",1);
      isclearbuf = 0;      
    }
    MOTIONVALUE = 1;
    ...
    if (MOTIONTIME>0){
      ...
      if (MOTIONTIME <=0){
         if (MOTIONVALUE == 1){
          sprintf(buf, "%sdevice[]=%s&value[]=%i&", buf, "motion1",0);
                 isclearbuf = 0;
  ...
  motion_value = digitalRead(MOTIONPIN2);
  if (motion_value == 1){
    MOTIONTIME2 = MOTIONDELAY2;
    if (MOTIONVALUE2 == 0){
      sprintf(buf, "%sdevice[]=%s&value[]=%i&", buf, "motion2",1);
      isclearbuf = 0;      
    }
    MOTIONVALUE2 = 1;
    ...
    if (MOTIONTIME2>0){
      ...
      if (MOTIONTIME2 <=0){
         if (MOTIONVALUE2 == 1){
          sprintf(buf, "%sdevice[]=%s&value[]=%i&", buf, "motion2",0);
                 isclearbuf = 0;
  ...
  LIGHTTIME2++;
  if (LIGHTTIME2 >= LIGHTDELAY2) {
     LIGHTTIME2 = 0;
     
     TEMPVALUE = analogRead(LIGHTPIN2);
     if (( TEMPVALUE<100 && abs(TEMPVALUE - LIGHTVALUE2)>30) || ( TEMPVALUE>=100 && abs(TEMPVALUE - LIGHTVALUE2)>50)) {  
        sprintf(buf, "%sdevice[]=%s&value[]=%i&", buf, "light2",(int)TEMPVALUE); 
           isclearbuf = 0;   
        LIGHTVALUE2 = TEMPVALUE;
     }
   }
  ...
  if (irrecv2.decode(&results)) // Если данные пришли 
  {

    storeCode(&results, "ir2");
    irrecv2.resume(); // Принимаем следующую команду
  }
...
void storeCode(decode_results *results, char* device) {
  ...
  //  Serial.println("Received unknown code, saving as raw");
   ...
   //     Serial.println("repeat; ignoring.");
    ...
    int res = checkIRCode(String(tbuf), String(device));
    if (res){
      sprintf(buf, "%sdevice[]=%s&value[]=%i_%lu_1&", buf, device, codeType,(int)codeValue);   
    }else{
      sprintf(buf, "%sdevice[]=%s&value[]=%i_%lu_0&", buf, device, codeType,(int)codeValue);   
    }
    isclearbuf = 0;
  }
...
int checkIRCode(String command, String device){

 
  for (int i=0; i<IRCOMMANDSSIZE; i++){

        
      if (IRACTIONS[i].indexOf(command)!=-1 && IRACTIONS[i].indexOf(device)==0){
